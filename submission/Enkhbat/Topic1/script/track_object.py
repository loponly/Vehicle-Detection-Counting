...
flags.DEFINE_string('video', './data/videos/Topic23-video1.mp4',
                    'path to input video or set to 0 for webcam')
flags.DEFINE_string(
    'output', './detection_track/Topic23-video1.mp4', 'path to output video')
...
def main(_argv):
    ...
    out = None
    output_name = '.'.join(FLAGS.output.split('.')[0:-1])
    in_destination = output_name+'_in_destination.json'

    output_log = output_name+'.txt'

    with open(in_destination, 'w') as f:
        f.write('\n')
    ...
    while True:
        ...
        if FLAGS.count:
            ...
            cv2.putText(frame, f"Moved to destination point: {len(to_dest)}", (
                5, 65), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (0, 255, 0), 2)

            cv2.putText(frame, f"destination point:", (
                258, 401), cv2.FONT_HERSHEY_COMPLEX_SMALL, 2, (0, 255, 0), 2)

            cv2.rectangle(frame, (258, 441), (260, 526), (0, 255, 0), 2)
        ...
        # if enable info flag then print details about each track
            if FLAGS.info:
                with open(output_log, 'a') as f:
                    textline = "Tracker ID: {}, Class: {},  BBox Coords (xmin, ymin, xmax, ymax): {}\n".format(
                        str(track.track_id), class_name, (int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3])))
                    f.write(textline)

            if int(bbox[0]) <= 258 and int(bbox[1]) <= 480:
                to_dest[track.track_id] = {
