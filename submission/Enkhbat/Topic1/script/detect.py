...
flags.DEFINE_string(
    'images_folder', './datasets/predict', 'path to input image')
...
def main(_argv):
    ...
    images = []
    for a, _, image_paths in os.walk(FLAGS.images_folder):
        for _image_path in image_paths:
            images.append(str(os.path.join(a, _image_path)))
    ...
    for count, image_path in enumerate(images, 1):
        ...
        count_of_object = {'car': 0, 'bus': 0, 'truck': 0}

        for i in range(num_boxes[0]):
            class_ind = int(out_classes[0][i])
            class_name = allowed_classes[class_ind]
            if class_name in count_of_object:
                count_of_object[class_name] += 1
        ...
        with open(f'{FLAGS.output}{file_name}.txt', 'w') as f:
            f.write(json.dumps(count_of_object))

        image = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)
        cv2.imwrite(FLAGS.output + file_name + '.png', image)
