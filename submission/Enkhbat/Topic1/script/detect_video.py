...
flags.DEFINE_string('video', './data/videos/Topic23-video2.mp4',
                    'path to input video or set to 0 for webcam')
flags.DEFINE_string(
    'output', './detections_video/Topic23-video2.mp4', 'path to output video')
...
def main(_argv):
    ...
    if FLAGS.output:
        ...
        with open(f'detections_video/{video_name}.txt', 'w') as f:
            f.write('\n')
    frame_num = 0
    while True:
        ...
        if FLAGS.count:
            ...
            with open(f'detections_video/{video_name}.txt', 'a') as f:
                f.write(json.dumps(
                    {**counted_classes, **{'fps': fps}})+'\n')
